...
public class ConveyorBelt : MonoBehaviour
{
	...
	void OnTriggerEnter(Collider other) {
		...
		if(other.gameObject.GetComponent<Rigidbody>() != null && other.gameObject.GetComponent<Rigidbody>().isKinematic == false && other.gameObject.tag != "Player" && pushingObjects.Contains(other.gameObject) == false){
            //Debug.Log("an object with a rigidbody just got added");
            pushingObjects.Add(other.gameObject);
        }
        if(other.transform.parent != null){
            if(other.transform.parent.gameObject.GetComponent<Rigidbody>() != null  && other.transform.parent.gameObject.GetComponent<Rigidbody>().isKinematic == false && other.transform.parent.gameObject.tag != "Player" && pushingObjects.Contains(other.transform.parent.gameObject) == false){
	...
	void OnTriggerExit(Collider other) {
        if(other.gameObject.GetComponent<Rigidbody>() != null && other.gameObject.GetComponent<Rigidbody>().isKinematic == false && other.gameObject.tag != "Player" && pushingObjects.Contains(other.gameObject) == true){
    ...
    void FixedUpdate()
    {
        for (int i = 0; i < pushingObjects.Count; i++){
	        ...
	        //if(pushingObjects[i].tag == "Player"){
	        //	if(pushingObjects[i].GetComponent<playerStates>().crouching){
	        //        Debug.Log("REMOVED VIA DISABLE");
	        //        pushingObjects.Remove(pushingObjects[i].gameObject);
	        // 	}
	        }
